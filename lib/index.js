  ...
  inspect: function (root, targetFile, options) {
    ...
    var job = new Promise(function parseFileContents(resolve, reject) {
      ...
      if (manifestType !== 'dotnet-core') {
        ...
        var sorted = {}
        Object.keys(flattenedPackageList).sort().forEach(function (key) {
          sorted[key] = flattenedPackageList[key];
        });
        flattenedPackageList = sorted;
      }
    })
    .then(function fetchNugetInformationFromPackages() {
      ...
      for (var name in flattenedPackageList) {
        var dep = flattenedPackageList[name];
        debug('...' + name);
      ...
      if (_nugtKeyCount > 0) {
        ...
        for (var key in nuspecResolutions) {
          ...
          var node = flattenedPackageList[resolution.name].cloneShallow();
